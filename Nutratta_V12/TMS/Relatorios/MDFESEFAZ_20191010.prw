	...
	For nX := 1 To Len(aVeiSF2)
		If HasTemplate("DCLEST")	//ExistTemplate("OMSA200P")
			...
			If MsSeek(xFilial("LBW")+PADR(aVeiSF2[nX],Len(LBW->LBW_PLACA)))
				aadd(aVeiculo,{})
				aadd( aVeiculo[Len(aVeiculo)] , Iif(LBW->(FieldPos("LBW_PLACA")) > 0 ,LBW->LBW_PLACA,""))
				aadd( aVeiculo[Len(aVeiculo)] , Iif(LBW->(FieldPos("LBW_PLACA")) > 0 ,LBW->LBW_PLACA,""))				
				aadd( aVeiculo[Len(aVeiculo)] , Iif(LBW->(FieldPos("LBW_UF")) > 0 ,LBW->LBW_UF,""))					
				aadd( aVeiculo[Len(aVeiculo)] , Iif(LBW->(FieldPos("LBW_TARA")) > 0 ,LBW->LBW_TARA,""))				
				aadd( aVeiculo[Len(aVeiculo)],"")
				aadd( aVeiculo[Len(aVeiculo)] , Iif(LBW->(FieldPos("LBW_TRANSP")) > 0 ,LBW->LBW_TRANSP,""))
				aadd( aVeiculo[Len(aVeiculo)],"")
				aadd( aVeiculo[Len(aVeiculo)] , Iif(LBW->(FieldPos("LBW_FROVEI")) > 0 ,LBW->LBW_FROVEI,""))	//Frota 1-Própria;2-Terceiro;3-Agregado
				aadd( aVeiculo[Len(aVeiculo)] , Iif(LBW->(FieldPos("LBW_CAPTOT")) > 0 ,LBW->LBW_CAPTOT,""))	//Capacidade Máxima
				aadd( aVeiculo[Len(aVeiculo)] , Iif(LBW->(FieldPos("LBW_ALTINT")) > 0 ,LBW->LBW_ALTINT,""))	//Altura Interna
				aadd( aVeiculo[Len(aVeiculo)] , Iif(LBW->(FieldPos("LBW_LARINT")) > 0 ,LBW->LBW_LARINT,""))	//Largura Interna
				aadd( aVeiculo[Len(aVeiculo)] , Iif(LBW->(FieldPos("LBW_COMINT")) > 0 ,LBW->LBW_COMINT,""))	//Comprimento Interno
				aadd( aVeiculo[Len(aVeiculo)] , Iif(LBW->(FieldPos("LBW_TIPROD")) > 0 ,LBW->LBW_TIPROD,""))	//Tipo de Rodado
				aadd( aVeiculo[Len(aVeiculo)] , Iif(LBW->(FieldPos("LBW_TIPCAR")) > 0 ,LBW->LBW_TIPCAR,""))	//Tipo de Carroceria
				aadd( aVeiculo[Len(aVeiculo)] , Iif(LBW->(FieldPos("LBW_RENAVA")) > 0 ,LBW->LBW_RENAVA,""))	//Renavam
				...
				If nX == 1
					...
					aadd(aMotorista[Len(aMotorista)],Iif(LBW->(FieldPos("LBW_NOMMOT")) > 0 ,LBW->LBW_NOMMOT,""))
					aadd(aMotorista[Len(aMotorista)],Iif(LBW->(FieldPos("LBW_CPFMOT")) > 0 ,LBW->LBW_CPFMOT,""))
				EndIf
			Else
				dbSelectArea("DA3")
				dbSetOrder(1)
				...
				If DUT->( msSeek( xFilial( "DUT" ) + DA3->DA3_TIPVEI ) )
					aadd( aVeiculo[Len(aVeiculo)] , Iif(DUT->(FieldPos("DUT_TIPROD")) > 0 ,DUT->DUT_TIPROD,"") ) 	//Tipo de Rodado
					aadd( aVeiculo[Len(aVeiculo)] , Iif(DUT->(FieldPos("DUT_TIPCAR")) > 0 ,DUT->DUT_TIPCAR,"") ) 	//Tipo de Carroceria
				Else
