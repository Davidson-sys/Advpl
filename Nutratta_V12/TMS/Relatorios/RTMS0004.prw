...
//ESPECIFICO PARA LOGISTICA]
//RELATORIO TULIO SOMENTE TES COM MOVIMENTACAO DE ESTOQUE.

//-------------------------------------------------------------------
/*/{Protheus.doc} RTMS0004
Relatorio especifico para a Logistica
Novo relatório com locais de entrega.
...
±±³Programa  ³_ReportDef ³ Autor ³ Marco Bianchi         ³ Data ³ 04/07/06 ³±±
...
TRCell():New(oPedNEnt,"DATREAL"	,"TRB"	,RetTitle("C6_ENTREG")	,PesqPict("SC6","C6_ENTREG"	),nTamData				      ,/*lPixel*/ ,{|| TRB->DATREAL			})	// Data de Entrega
...
TRCell():New(oCliente,"DATREAL"		,"TRB",RetTitle("C6_ENTREG"		),PesqPict("SC6","C6_ENTREG"	),TamSx3("C6_ENTREG"	)[1],/*lPixel*/,{|| TRB->DATENTR																			})
...
//TRCell():New(oCliente,"VENDEDOR"	,"TRB",RetTitle("C5_VEND1"		),PesqPict("SC5","C5_VEND1"		),TamSx3("C5_VEND1"	)[1],/*lPixel*/,{|| TRB->VENDEDOR																					})
//TRCell():New(oCliente,"NOMEVEN"	,"TRB",RetTitle("A3_NOME"		),PesqPict("SA3","A3_NOME"		),TamSx3("A3_NOME"	)[1],/*lPixel*/,{|| TRB->NOMEVEN	
...
TRCell():New(oData,"DATREAL"	,"TRB",RetTitle("C6_ENTREG"		),PesqPict("SC6","C6_ENTREG"	),TamSx3("C6_ENTREG"		)[1],/*lPixel*/,{|| TRB->DATENTR																			})
...
//				oSecao:Cell("NOMEVEN"	):Hide()
				...
				oSecao:Cell("CLIENTE"	):Hide() //:Hide()
				oSecao:Cell("LOJA"		):Hide()
				oSecao:Cell("NOMECLI"	):Show() //:Hide()
				oSecao:Cell("NUM"		):Show()
				oSecao:Cell("ITEM"		):Show()
				oSecao:Cell("EMISSAO"	):Show()
				oSecao:Cell("PRODUTO1"	):Show()
...
AADD(aCampos,{ "DATREAL"   ,"D",aTam[1],aTam[2] } )
aTam:=TamSX3("C6_ITEM")
...
#IFDEF TOP                                                   
	...
	WHERE C6_FILIAL = %xFilial:SC6%
		...
		AND C6_ENTREG >= %Exp:"20170101" % AND C6_ENTREG <= %Exp:'20201231'%
...
#ELSE
	...
	cCondicao  += "Dtos(C6_ENTREG)>= '20170101' .And. Dtos(C6_ENTREG)<= '20501231 "
...
//Tratamento para buscar a data real de entrega.
/*dbSelectArea("SC5")
dbSetOrder(1)
If dbSeek((cAliasQry)->C6_FILIAL+(cAliasQry)->C6_NUM)

	dDtEntreg:=SC5->C5_FECENT		
EndIf
*/
...
dDtEntreg:=	POSICIONE("SC5",1,XFILIAL("SC5")+(cAliasQry)->C6_NUM,"C5_FECENT")

//Tratamento para sair caso ja tenha atingido a data final.
If dDtEntreg > MV_PAR08
	dbSelectArea(cAliasQry)	
	dbSkip()                                                              
	Loop
EndIf
	
If !Empty(dDtEntreg) .And. dDtEntreg  >=  MV_PAR07 .And. dDtEntreg <= MV_PAR08 
	               
//	 Alert("entrou")
	//--Não imprimir pedidos com residuos eliminados. 
	cResiduos	:=	Posicione("SC5",1,(cAliasQry)->C6_FILIAL+(cAliasQry)->C6_NUM,"C5_NOTA")
	If cResiduos $ 'XXXXXXXXX'	
		dbSelectArea(cAliasQry)	
   	dbSkip()                                                              
	   Loop
	EndIf
	
	//--Não imprimir caso a tes utilizada nao atualize estoque. 
	cGerEst:= Alltrim(Posicione("SF4",1,(cAliasQry)->C6_FILIAL+(cAliasQry)->C6_TES,"F4_ESTOQUE"))
	If cGerEst == 'N'	
		dbSelectArea(cAliasQry)	
   	dbSkip()
	   Loop
			...
			For nX := 1 to nQtdVend
				...
				If !EMPTY(cVendedor) .And. (cVendedor >= MV_PAR16 .And. cVendedor <= MV_PAR17 )
					...
					If (Empty(cFilSA3)) .Or. (!Empty(cFilSA3) .And. (&cFilSA3))
						...
						Replace DATREAL    With  dDtEntreg
...
PRIVATE aReturn := { STR0005, 1,STR0006, 1, 2, 1, "", IIf(isBlind(), val(SX4->X4_ORDEM), 1) }      //"Zebrado"###"Administracao"
PRIVATE nTamRef := Val(Substr(GetMv("MV_MASCGRD"),1,2))
...
While !Eof() .And. (cAliasSC6)->C6_FILIAL == xFilial("SC6")
	...
	If ( Empty(aReturn[7]) .Or. &(aReturn[7]) )
		...
		If Alltrim((cAliasSC6)->C6_BLQ) == "R" .Or. mv_par13 == 2 // Se Foi Eliminado Residuos
			nSaldo  := 0
		Else
			nSaldo  := C6_QTDVEN-C6_QTDENT
		Endif
