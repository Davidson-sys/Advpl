...
//Ponto de Entrada no Cálculo do Frete, retorna .T. ou .F. e impede ou não a continuação do cálculo.
//Objetivo: Evitar erros nos cadastros dos clientes remetentes e destinatários para:
//1) Evitar documentos inutilizados na SEFAZ.
//2) Evitar documentos com cálculos errados e estornos de CTe´s.
User Function TM200OK()
	...
	If nOpc == 1	//Cálculo do Frete
		DbSelectArea("SA1")
		SA1->(DbSetOrder(1))
		//Validações no cadastro do cliente remetente
		If SA1->(DbSeek(xFilial("SA1")+cCliRem+cLojRem))
			...
			*/
			...
			If (Posicione("DUY",1,xFilial("DUY")+SA1->A1_CDRDES,"DUY_EST")) <> SA1->A1_EST
				cMsg += "- O código da região do cliente não pertence ao mesmo estado que o cliente."+CHR(13)+CHR(10)
				lCtrlRem := .T.
				lRet := .F.
			Endif
		Endif
		...
		//Validações no cadastro do cliente destinatário
	...
	If nOpc == 3	//Estorno. Ajustando o SX3 para corrigir o error.log até a virada para P12 ou até a TOTVS resolver o chamado aberto.
		DbSelectArea("SX3")
		SX3->(DbGoTop())
		While SX3->(!Eof())
