...
5949-OUTRA SAIDA DE MERC. OU PREST. SERV. NAO ESPECIFICADO  
6553-DEVOLUCAO DE COMPRA DE BEM PARA O ATIVO IMOBILIZADO    
...
*/
//-------------------------------------------------------------------
/*/{Protheus.doc} N_MATREST
Relatório Faturamento especifico Nutratta.
...
Local cTesEst 	:= GetMv("MV_MOVEST") //Parametro com as TES que movimentam estoque.
...
#IFDEF TOP
    ...
    If TcSrvType()<>"AS/400"
		...
		WHERE F2_FILIAL  BETWEEN "0101"  AND  "0103"	//%xFilial:SF2%
			...
			AND SF2.%notdel%
			AND D2_FILIAL BETWEEN "0101" AND "0103"		 //%xFilial:SD2% 
			AND D2_CLIENTE = F2_CLIENTE AND D2_LOJA = F2_LOJA
			AND D2_DOC = F2_DOC AND D2_SERIE = F2_SERIE
//			AND D2_COD IN ('25000002','25000007','25000005','21000011','21000004','21000023','21000022','21000066','21000065','21000010',;
//							'21000003','21000012','21000008','21000015','21000017','21000013','21000034','21000066','21000065')
			AND SD2.%notdel%       
			%Exp:cWhere%	
...
While !oReport:Cancel() .And. !(cAliasQry)->(Eof())
	...
	//Valida se a TES utilizada esta dentro do parametro
	If Alltrim((cAliasQry)->D2_TES) $ cTesEst 
...
Local cExprGrade:= ""  
Local cGerFin	:=""
Local cFops		:= SuperGetMv("NT_CFVENDA",.F.,"5101") //GETMV("NT_CFVENDA") "5101,5102,5105,5106,6101,6102,6105,6106,6107,6108,6111,6551,5551,5949" 
...
#IFNDEF TOP
	oReport:Section(2):Section(1):Cell("NACN6"		):SetBlock({||IIF((cAliasSD2)->D2_TIPO $ "P",xMoeda((cAliasSD2)->D2_VALIPI,1,MV_PAR13,(cAliasSD2)->D2_EMISSAO),xMoeda((cAliasSD2)->D2_TOTAL+(cAliasSD2)->D2_VALIPI+(cAliasSD2)->D2_ICMSRET,1,MV_PAR13,(cAliasSD2)->D2_EMISSAO, TAMSX3("D2_TOTAL")[2]))})	
#ELSE
	oReport:Section(2):Section(1):Cell("NACN6"		):SetBlock({||IIF((cAliasQry)->D2_TIPO $ "P",xMoeda((cAliasQry)->D2_VALIPI,1,MV_PAR13,(cAliasQry)->D2_EMISSAO),xMoeda((cAliasQry)->D2_TOTAL+(cAliasQry)->D2_VALIPI+(cAliasQry)->D2_ICMSRET,1,MV_PAR13,(cAliasQry)->D2_EMISSAO, TAMSX3("D2_TOTAL")[2]))})
#ENDIF
...
#IFDEF TOP
    If TcSrvType()<>"AS/400"		
		...
		EndIf
		cWhere += " AND NOT ("+IsRemito(2,"F2_TIPODOC")+")"
		cWhere += " AND D2_CF IN ("+cFops+")" //AND D2_CF IN "() %Exp:cFops%
		cWhere +="%"
...
cGerFin:= Posicione("SF4",1,xFilial("SF4")+Alltrim((cAliasQry)->D2_TES),"F4_DUPLIC")
//cGerFin:= Posicione("SF4",1,(cAliasQry)->F2_FILIAL+(cAliasQry)->D2_TES,"F4_DUPLIC")
	//	cFnoTa	:= Alltrim(Posicione("SD2",3,(cAliasSF2)->(F2_FILIAL+F2_DOC+F2_SERIE),"D2_CF"))	
	
	//Não exibir no relatorio registros com a CFOP 5949 e sem geração de titulos financeiros
If cGerFin $ "S"  //cCfOp == "5949" .And.
...
WHERE	F1_FILIAL BETWEEN "0101"  AND  "0103"	//= %xFilial:SF1%
...
#IFDEF TOP
    ...
    If TcSrvType()<>"AS/400"
		...
		cQuery+="F2_FILIAL F2_FILIAL  BETWEEN '0101'  AND  '0103' "
		...
		cQuery+=" AND D2_FILIAL BETWEEN '0101' AND '0103'  AND D2_CLIENTE=F2_CLIENTE AND D2_LOJA=F2_LOJA"
		cQuery+=" AND D2_DOC=F2_DOC AND D2_SERIE=F2_SERIE"
		cQuery+=" AND D2_COD IN ('25000002')"	// >='"+mv_par05+"' AND D2_COD<='"+mv_par06+"'"
		cQuery+=" AND D2_CF  IN ('5101')"	// >='"+mv_par05+"' AND D2_COD<='"+mv_par06+"'"
