...
Ponto de entrada na gravação do cadastro de cliente
...
//-----------------------------------------------------------------------------------
// Verificar se o usaurio do parametro é o mesmo do usuario logado na rotina
...
If cCodUsr $ cUsuCli
	...
	ElseIf Empty(M->A1_END)
		MsgInfo ("Gentileza informar o campo ENDEREÇO do cliente!!!")
		lExecuta := .F.                                              
		Return (lExecuta)
...
//-----------------------------------------------------------------------------------
// Validação para os usuarios que não estão no parametro.
//-----------------------------------------------------------------------------------
Else  
	If Empty(M->A1_lOJA)
		MsgInfo ("Gentileza informar o campo LOJA!!!")
	...
	ElseIf Empty(M->A1_INSCR )
		MsgInfo ("Gentileza informar o campo INSCRIÇÃO ESTADUAL do cliente!!!")
		lExecuta := .F.                                              
		Return (lExecuta)		
	...
	ElseIf Empty(M->A1_CODPAIS )
		...
		lExecuta := .F.                                              
		Return (lExecuta) 
	//-Quando o cliente for ISENTO o campo contribuinte na aba fiscal deverá estar como não.		
	ElseIf !IsNumeric(Alltrim(StrTran(StrTran(M->A1_INSCR,'.',''),'-','')))
		If Alltrim(M->A1_CONTRIB) == "1"	//1=Sim;2=Nao                                                                                                                      
			MsgInfo ("Para clientes com inscricao isentas o campo contribuinte deverá ser não!!!")
			lExecuta := .F.                                              
			Return (lExecuta)
		EndIf
	EndIf		     
