...
oReport:= TReport():New("RNUTF01","Relação de Pedidos Nao Entregue","RNUTF01", {|oReport| ReportPrint(oReport)},"Relação de Pedidos não Entregue") //"Relacao de Pedidos de Compras"##"Emissao da Relacao de  Pedidos de Compras."##"Sera solicitado em qual Ordem, qual o Intervalo para"##"a emissao dos pedidos de compras."
...
TRCell():New(oSection1,"CDESPROD"	,"","Descrição Produto"  	,"@!"   					,40,/*lPixel*/	,{|| cDesProd })
...
TRCell():New(oSection1,"CSERNOTA"	,"","Série NF-e"         	,"@!"						,03,/*lPixel*/	,{|| cSerNota })
...
//TRCell():New(oSection1,"CCONDPAG"	,"","Condição"		     	,"@!"	  					,03,/*lPixel*/	,{|| cCondpag })
//TRCell():New(oSection1,"CDESCPAG"	,"","Descrição"		     	,"@!"	  					,40,/*lPixel*/	,{|| cDescpag })
...
PutSx1(cPerg,"09","Status Pedidos?" ," "," "   ,"mv_ch9","N",1,0,1,"C","","","","","mv_par09","Entregues","Sí","Yes","","Não Entregues","No","No","Ambos","Ambos","Ambos","","","","","","")
PutSx1(cPerg,"10","Seleciona Filiais?" ," "," ","mv_ch10","N",1,0,1,"C","","","","","mv_par10","Sim","Sí","Yes","","Não","No","No","","","","","","","","","")
//PutSx1(cPerg,"01","Filtrar emissão por  ?" 		," "," "   ,"mv_ch1","N",1,0,1,"C","","","","","mv_par01","Pedidos","Sí","Yes","","Notas ","No","No","","","","","","","","","")
//PutSx1(cPerg,"08","Seleciona Vendedores?" ," "," ","mv_ch7","N",1,0,1,"C","","","","","mv_par07","Sim","Sí","Yes","","Não","No","No","","","","","","","","","")
...
//-- Davidson 07/07/2017.
//-- Filtra por data de emissão Pedido de vendas.

//Emissão e Pedidos entregues
If MV_PAR09 == 1 

	cQrySC5 += " AND SC5.C5_TIPO = 'N'"
	cQrySC5 += " AND SC5.C5_NOTA <> 'XXXXXXXXX' " //Adicionado tratamento Elimminação de residuos Davidson-25/10/2016.
...
//Emissão Pedidos não entregues
ElseIf MV_PAR09 == 2                   

	cQrySC5 += " AND SC5.C5_TIPO = 'N'"
	cQrySC5 += " AND SC6.C6_NOTA = '' AND C6_SERIE = '' AND SC5.C5_NOTA ='' " //Adicionado tratamento Elimminação de residuos Davidson-25/10/2016.
	cQrySC5 += " AND SC5.C5_EMISSAO BETWEEN '"+dToS(Mv_Par01) +"' AND '"+dToS(Mv_Par02)+"'"   //Emissao do pedido
	cQrySC5 += " AND SC5.C5_FECENT BETWEEN '"+dToS(Mv_Par05) +"' AND '"+dToS(Mv_Par06)+"'"    //Data de entrega
//	cQrySC5 += " AND SC6.C6_DATFAT BETWEEN '"+dToS(Mv_Par03) +"' AND '"+dToS(Mv_Par04)+"'"    //Faturamento da NFe  
//	cQrySC5 += " AND SC6.C6_ENTREG  BETWEEN '"+dToS(Mv_Par05) +"' AND '"+dToS(Mv_Par06)+"'

//Emissão Pedidos Pedidos entregues e não entregues=Ambos.
ElseIf MV_PAR09 == 3
...
/*
//--Filtra por data de emissão da nota fiscal . 
ElseIf cFilt == 2  
...
If MV_PAR10 == 1 //Pedidos entregues
	cQrySC5 += " AND SC6.C6_NOTA <> '' AND C6_SERIE <> '' "
ElseIf MV_PAR10 == 2 //Pedidos não entregues
	cQrySC5 += " AND SC6.C6_NOTA = '' AND C6_SERIE = '' AND SC5.C5_NOTA ='' " //Adicionado tratamento Elimminação de residuos Davidson-25/10/2016.
ElseIf MV_PAR10 == 3 //Ambos --Adicionado tratamento Elimminação de residuos Davidson-28/11/2016.
	cQrySC5 += " AND SC5.C5_NOTA <> 'XXXXXXXXX' " //Adicionado tratamento Elimminação de residuos Davidson-25/10/2016.
EndIf																				 
*/
	...
	If lRet .And. lConsolida
		For nX := 1 To Len(aFilsCalc)
			...
			If !Empty(nPos) .And. aFilsCalc[nX,1] # aEmpresas[nPos,2]
				If Empty(nOpca)
					If lMostraTela
						nOpca := Aviso("Atenção","A execução desta rotina foi parametrizada para modo consolidado porém não foram selecionadas todas as filiais de uma ou mais empresas. Deseja que estas filiais sejam adicionadas a seleção ou mantém a seleção atual?",{"Teste 1","Teste 2","Teste 3"},2) //"A execução desta rotina foi parametrizada para modo consolidado porém não foram selecionadas todas as filiais de uma ou mais empresas. Deseja que estas filiais sejam adicionadas a seleção ou mantém a seleção atual?"
					Else
						nOpca := 1
					EndIf
				EndIf
	 	  	...
	 	  	Else
		 	  	...
		 	  	ElseIf nValida == 2
		 	  	   //'SIGACUSCNPJIE' ; 'A Consolidação por CNPJ+IE está habilitado. Selecione apenas Filiais com o mesmo CNPJ+IE [' ; '] já marcado, ou refaça sua seleção!'
		 	  		Help(nil,1,'SIGACUSCNPJIE',nil,'A Consolidação por CNPJ+IE está habilitado. Selecione apenas Filiais com o mesmo CNPJ+IE ['+Transform(Substr(cSelCNPJIE,1,14),"@R 99.999.999/9999-99")+" - "+Substr(cSelCNPJIE,15)+'] já marcado, ou refaça sua seleção!',3,0)
		 	  	ElseIf nValida == 3
...
Data------: 12/04/2018
========================================================================================================================
Descrição-: Atualiza as datas de entrega antes da emissão do relatorio
