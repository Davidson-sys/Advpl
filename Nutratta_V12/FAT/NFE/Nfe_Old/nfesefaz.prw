	...
	For nCountNF := 1 To Len(aCupRefLoj)
		...
		//						If MsSeek(xFilial("SD2")+(cAliasSD2)->D2_NFORI+(cAliasSD2)->D2_SERIORI+(cAliasSD2)->D2_CLIENTE+(cAliasSD2)->D2_LOJA+(cAliasSD2)->D2_COD+(cAliasSD2)->D2_ITEMORI)
						...
						While CD2->(!Eof()) .And. xFilial("CD2") == CD2->CD2_FILIAL .And.;
							...
							ElseIf SFT->(FieldPos("FT_TRFICM")) <> 0 .And. SFT->FT_TRFICM <> 0 .And. IIF(!lEndFis,ConvType(SM0->M0_ESTCOB),ConvType(SM0->M0_ESTENT)) == "RS"
							...
							Do Case
								...
								Case AllTrim(CD2->CD2_IMP) == "SOL"
									...
									Iif(cVerAmb == "4.00" .and. FindFunction("xFisRetFCP"), xFisRetFCP('4.0','CD2','CD2_ALIQ'), If(lNfCupZero,0,If(lNfCupZero,0,CD2->CD2_ALIQ))),; 
									Iif(cVerAmb == "4.00" .and. FindFunction("xFisRetFCP"), xFisRetFCP('4.0','CD2','CD2_VLTRIB'), If(lNfCupZero,0,CD2_VLTRIB)),; 
									CD2->CD2_MVA,;
									CD2->CD2_QTRIB,;
									CD2->CD2_PAUTA,;
									Iif(lCD2PARTIC,CD2->CD2_PARTIC,""),;
									...
									If lConsig .And. (Alltrim((cAliasSD2)->D2_CF) $ cMVCFOPREM)  .And. CD2->CD2_VLTRIB > 0 
										aTail(aICMSST):= {CD2->CD2_ORIGEM,CD2->CD2_CST,CD2->CD2_MODBC,0,0,0,0,CD2->CD2_MVA,0,CD2->CD2_PAUTA,Iif(lCD2PARTIC,CD2->CD2_PARTIC,""),IIf(CD2->(ColumnPos("CD2_DESONE"))>0,CD2->CD2_DESONE,0)}
									EndIf
									lCalSol := .T.
									//ÚÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ¿
									...
									If !lIcmSTDev
										If ( (cAliasSD2)->D2_TIPO=="D" .Or. ( (cAliasSD2)->D2_TIPO=="I" .And. lComplDev)) .And. !Empty(nValST) 
											...
											nValST 	  := 0
											aTail(aICMSST):= {CD2->CD2_ORIGEM,CD2->CD2_CST,CD2->CD2_MODBC,0,0,0,0,CD2->CD2_MVA,	CD2->CD2_QTRIB,CD2->CD2_PAUTA,Iif(lCD2PARTIC,CD2->CD2_PARTIC,""),IIf(CD2->(ColumnPos("CD2_DESONE"))>0,CD2->CD2_DESONE,0)}
										EndIf
									EndIf
								...
								Case AllTrim(CD2->CD2_IMP) == "TST" 
									...
									aImp := {;
												...
												IIf(!Empty(SC5->C5_MUNPRES),SC5->C5_MUNPRES,0),;	//cMunFG - Código do município de ocorrência do fato gerador do ICMS do transporte
			...
			While !Eof() .And. xFilial("SD1") == (cAliasSD1)->D1_FILIAL .And.;
		       ...
		       EndIf   		
				...
				aadd(aProd,	{Len(aProd)+1,;  
					...
					(cAliasSD1)->D1_DESPESA + (cAliasSD1)->D1_VALPS3 + (cAliasSD2)->D2_VALCF3 + nIcmsST + IIF( ((cAliasSD1)->D1_TIPO == "D" .And. !lEipiDev) .Or. ((cAliasSD1)->D1_TIPO == "B" .and. lIpiBenef),(cAliasSD1)->D1_VALIPI,0),;//Outras despesas + PISST + COFINSST  (Inclusão do valor de PIS ST e COFINS ST na tag vOutros - NT 2011/004).
					nRedBC,;//% Redução da Base de Cálculo
					cCST,;//Cód. Situação Tributária
...
If !Empty(aNota)
	...
	IF cVeramb >= "4.00"
		cString += NfePag(aNota,aTotal,aDupl,cTPNota,AdetPag)
	EndIf
...
Do Case
	...
	Case (cVeramb >= "3.10" .and. (!Empty(aNfVinc) .Or. !Empty(aRefECF) .Or. !Empty(aNfVincRur))) .and. ( (aNota[5] $ "D|B" .And. SF4->F4_AJUSTE <> "S") .or. (aNota[5] $ "N" .And. SF4->F4_PODER3 == "D") )
     ...
     documento fiscal referenciado conforme  NT 2013/005 v 1.20.
   	 */
    Case (aNota[5]) $ "D" .and. Empty(aNfVinc).and. Alltrim(SFT->FT_CFOP) $ "1201-1202-1410-1411-5921-6921-2201"
   	 	cTPNota:= "4" 
...
If  !lIssQn    
	...
	Else		
		...
		If Len(aIcms)>0
			...
			IF cVeramb >= "4.00" .and. aCST[1] $'00,20,41,51,90' 
			   IF  aCST[1] <> '00'
	...
	If Len(aIcmsST)>0	
		...
		//Criação de campos relativos ao FCP (Fundo de Combate à Pobreza) para operações internas ou interestaduais com ST.
		IF cVeramb >= "4.00" .and. aCST[1] $'10,20,30,60,70,90' //.and. !Empty(aICMS[13])
				cString += '<vBCFCPST>'+ConvType(aICMSST[13],15,2)+'</vBCFCPST>'
				cString += '<pFCPST>'+ConvType(aICMSST[14],5,2)+'</pFCPST>'
				cString += '<vFCPST>'+ConvType(aICMSST[15],15,2)+'</vFCPST>'	
		EndIf	
	...
	ELse
		...
		If aCST[01] = "60" .AND. cTipo=="1"		
			...
			If nBaseIcm > 0 .and. nValICM>0
				...
				ElseIf IIF(!lEndFis,ConvType(SM0->M0_ESTCOB),ConvType(SM0->M0_ESTENT)) <> "MG" 
					If lCalcMed
						nBaseIcm := aProd[09]*nBaseIcm
						nValICM  := aProd[09]*nValICM
					EndIf
...
If lSuframa .And. (IIF(!lEndFis,ConvType(SM0->M0_ESTCOB),ConvType(SM0->M0_ESTENT)) == "PA") .And. !empty(aDest[15])
	cMensFis += "Código Suframa: "+alltrim(aDest[15]+".") 
Endif
...
Do Case
  ...
  //” acompanhada do respectivo valor.
	...
	IF cVerAmb == "4.00" .and. cDestDanf =='2'
  		If Len(aICMS) > 0 
      		If  (aICMS[16] > 0 .or. aICMS[17] > 0 .or.  aICMS[18] > 0)
  				cMensFcop := AllTrim("(FCP): Base R$ "+ConvType(aICMS[16],13,2)+" Perc.("+ConvType(aICMS[17])+"%) Vlr. R$ " + ConvType(aICMS[18],13,2))  
  			EndIf
  		EndIf
  	If Len(aICMSSt) > 0 
  			If  (aICMSST[13] > 0 .or. aICMSST[14] > 0 .or.  aICMSST[15] > 0)
  				cMensFcop += AllTrim("  (FCPST): Base R$ "+ConvType(aICMSST[13],13,2)+" Perc.("+ConvType(aICMSST[14])+"%) Vlr. R$ " + ConvType(aICMSST[15],13,2))
  			EndIf	
  		EndIf	    
  	EndIf    								
...
Static Function NfePag(aNota,aTotal,aDupl,cTPNota,AdetPag)
Local cString := ""
Local nX := 0  
local cForma :="14" //14=Duplicata Mercantil  - 857 Rejeição: Informado Duplicata Mercantil como Forma de Pagamento e não preenchido o Grupo Duplicata de Pagamento
Default aNota	:= {}
Default aTotal:= {}
Default aDupl:= {}
Default AdetPag	:= {}

If	cTPNota $ '3-4' .and. Len(aDupl) == 0
	cForma:= '90'  //90=Sem Pagamento.
ElseIf Len(aDupl) == 0
	cForma:= '99'  //outros
ElseIf Len(aDupl) > 0	
   cForma :="14"  //14=Duplicata Mercantil  
Endif
	cString +='<pagamento>'
	cString +='<detPag>'
	cString +='<forma>'+cForma+'</forma>'
	cString +='<valor>'+ConvType(aTotal[2],15,2)+'</valor>'
	cString +='</detPag>'
	cString +='<troco>0.00</troco>'
	cString +='</pagamento>'
