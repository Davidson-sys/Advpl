...
If nTipo <> 1
	...
	Private cDadFidAB	:= SuperGetMv("MV_XBCORIA",.F.,.F.)		//MV_XBCORIA -Paramettro alternativo homologação. NT_BCORIAB Base OFC.
	Private cDadRDEF	:= SuperGetMv("NT_BCORIDF",.F.,.F.)
	Private lAtivFidc	:= SuperGetMv("MV_XATVFID",.F.,.T.)
	Private cTitulo 	:= "Emissão de Boletos Bancarios"
	Private cMens		:= "Deseja Imprimir os boletos em sequencia a cada DANFE?"
	Private cRet		:= ""
	Private nCnt		:= 0
...
If lExistNFe
	...
	//Chamada da Impressão do Boleto
	Private lImpBol:=	.F.
	
	//Verifica se a rotina RFIN001 esta sendo utilizada por outro usuario.
	...
	If lImprime
		...
		// Inicio Davidson
		Private cDadBco 	:= ""	// GETNEWPAR("NT_DADBCO", "") // -- Davis 23/02/2016
		Private cFuncao 	:= ""
		Private cCodCli		:=""
		Private cLjCli		:=""
		
		// -- Risco do cliente 31/01/2017
		//******************
		//Davidson Clayton *
		//******************
		dbSelectArea("SF2")
		...
		If dbSeek(xFilial("SF2")+MV_PAR01+MV_PAR03)
			...
			//-----------------------------------------------------------------------------------
			// Verifica o vinculo do portador posicionado.
			//-----------------------------------------------------------------------------------
			dbSelectArea("SA6")
			dbGotop()
			While !Eof()
				...
				If SA6->A6_FATPORT == "1" .And. SA6->A6_BLOCKED  == "2"
					...
					dbSelectArea("SX6")
					If dbSeek(xFilial("SX6")+&(SA6->A6_CXPOSTA))
						cFuncao 	:= Alltrim(SX6->X6_CONTSPA)+"("+Alltrim(SX6->X6_CONTENG)+")"
						&cFuncao
					EndIf
					exit
				EndIf
		...
		//- Busca o risco do cliente
		If ! Empty(cDadBco)
			cFuncao := Alltrim(SX6->X6_CONTSPA)+"("+Alltrim(SX6->X6_CONTENG)+")"
			//	MsgStop("Funcao com Parametro: "+cFuncao)
			&cFuncao
		Else
