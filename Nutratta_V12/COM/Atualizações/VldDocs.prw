...
Colocar no campo esta validação de usuario = u_VldDocs(cNFiscal)
...
@return  	n/t
@obs.......  
Validação chamada no campo F1_DOC 
Chamada na validação do usuario
U_VldDocs(cNFiscal)                                                                                                             
xxx......
/*/
//-----------------------------------------------------------------------------------------------------------
...
If FunName() == "MATA103"      
	...
	If dbSeek(cFilAnt+cFornec+cLoja+cNFiscal)	//F1_FILIAL+F1_FORNECE+F1_LOJA+F1_DOC
		Aviso("Atenção","Já existe um documento com mesmo número lançado para este fornecedor.",{"OK"},2) 
	EndIf
EndIf
Return (lRet)      